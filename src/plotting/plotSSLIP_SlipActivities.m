function caxisMinMax = plotSSLIP_SlipActivities(ebsdID,sSLocal,opt)
% Plot saved physical slip activities, in opt.NoSs order.
% Adapted from Philipp (PhilKro), PR #2, commit d3ee1a5:
% https://github.com/Tijmenvermeij/SSLIP/pull/2
% This first separation retains the existing activity maps, scales, traces,
% and export names. Grouping and dominant-system ranking are separate steps.
% Activity rows in ebsdID.prop.slipIDcor correspond to sSLocal(opt.NoSs).
% The optional output supplies the activity limits for shared residual plots.
